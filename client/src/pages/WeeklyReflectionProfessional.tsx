...
function WeeklyReflectionProfessionalContent() {
  const [selectedWeek, setSelectedWeek] = useState<string>("1");
  const [surprised, setSurprised] = useState("");
  const [applicationToProfessional, setApplicationToProfessional] = useState("");
  ...
  const handleSave = async () => {
    if (!surprised.trim() || !applicationToProfessional.trim() || !nextWeekTest.trim()) {
    ...
    try {
      await saveReflection.mutateAsync({
        weekNumber,
        surprised,
        applicationToFashion: applicationToProfessional,
        nextWeekTest,
      });
  ...
  if (reflection && !surprised) {
    ...
    setApplicationToProfessional(reflection.applicationToFashion || "");
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold flex items-center gap-2">
          ...
          Weekly Reflection - Professional Track
        </h1>
        <p className="text-muted-foreground mt-2">
          Answer these guided reflection prompts to deepen your professional learning at Philip Morris. This is your space to reflect on career growth.
        </p>
      </div>
      ...
      <div className="grid gap-6">
        ...
        {/* Card 2: Application to Professional Growth */}
        <Card className="border-blue-200 bg-gradient-to-br from-blue-50 to-transparent">
          <CardHeader>
            <CardTitle className="text-lg">💼 How Does This Apply to Your Professional Growth?</CardTitle>
            <CardDescription>
              Connect this week's learning to your career advancement at IQOS
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Textarea
              placeholder="Write your reflection here..."
              value={applicationToProfessional}
              onChange={(e) => setApplicationToProfessional(e.target.value)}
              className="min-h-32 bg-white"
            />
          </CardContent>
        </Card>
        ...
        <Card className="border-green-200 bg-green-50">
          ...
          <CardContent>
            <p className="text-sm text-green-700">
              Your week {weekNumber} professional reflection has been saved. Keep growing! 📈
            </p>
          </CardContent>
        </Card>
      )}
...
export default function WeeklyReflectionProfessional() {
  return (
    <DashboardLayout>
      <WeeklyReflectionProfessionalContent />
    </DashboardLayout>
  );
}
